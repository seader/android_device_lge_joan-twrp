...
while ! mountpoint  /data; do
	...
	if ! mountpoint /system; then #mount system if not already mounted
        ...
        if [ -f $system/vendor/bin/qseecomd ]; then
#                start sys_qseecomd
		if [ -z "$(pidof qseecomd)" ]; then #make sure it isnt already running
			start sys_vold
			LD_LIBRARY_PATH='$system/lib64:$system/lib:$system/vendor/lib:$system/vendor/lib64' PATH='$system/vendor/bin:$system/bin' $system/vendor/bin/qseecomd &
		fi
        else
#                start ven_qseecomd
		if [ -z "$(pidof qseecomd)" ]; then
			LD_LIBRARY_PATH='/vendor/lib64:$system/lib64:/vendor/lib:$system/lib' PATH='/vendor/bin:$system/bin' /system/bin/qseecomd &
		fi
        fi
	if [ ! -z "$GSI" ]; then
